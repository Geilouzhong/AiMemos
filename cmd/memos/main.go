...
func init() {
	...
	rootCmd.PersistentFlags().Bool("enable-mcp", false, "enable MCP server")
	rootCmd.PersistentFlags().String("mcp-path", "/mcp", "MCP server endpoint path")
	...
	if err := viper.BindPFlag("enable-mcp", rootCmd.PersistentFlags().Lookup("enable-mcp")); err != nil {
		panic(err)
	}
	if err := viper.BindPFlag("mcp-path", rootCmd.PersistentFlags().Lookup("mcp-path")); err != nil {
		panic(err)
	}
