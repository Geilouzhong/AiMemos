...
func initActivityLogger() error {
	activityLoggerOnce.Do(func() {
		...
		if err := os.MkdirAll("logs", 0755); err != nil {
			activityLoggerInitErr = errors.Wrap(err, "failed to create logs directory")
			return
		}
		...
		if err != nil {
			activityLoggerInitErr = errors.Wrap(err, "failed to open activity log file")
			return
		}
...
func logMemoActivity(userID int32, memoID string, username string, ip string, port int) error {
	...
	if _, err := activityLogFile.WriteString(logLine); err != nil {
		return errors.Wrap(err, "failed to write activity log")
	}
